...
class pageNavigator extends React.Component<PropsType, StateType> {
    ...
    public render(): JSX.Element {
        const navigatorView: Array<JSX.Element> = this.toNavigatorView(
            this.state.pageList.pages
        );

        return (
            <div className="pageNavigatorStyle">
                <Button
                    key={`${this.props.subkey}button-previous`}
                ...
                {navigatorView}
                <Button
                    key={`${this.props.subkey}button-next`}
                    onClick={this.onNextClicked}
                    content="Next"
                    color="blue"
                    size="medium"
                    compact
                    disabled={this.state.selectedPage === this.props.pageNum}
                />
            </div>
        );
    }
            ...
            if (idx > 0 && num - this.state.pageList.pages[idx - 1] !== 1) {
                return (
                    <div key={this.props.subkey + num}>
                        ...
                        <PageButton
                            key={`${this.props.subkey}button-${num}`}
                            pageNum={num}
